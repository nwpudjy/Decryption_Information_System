...
def add(W1,W2):#W1L||W2H
    W1l=L(W1)<<16
    W2h=H(W2)
    all=W1l|W2h
    return all

def add2(W1,W2):#W1H||W2L
    W1h = H(W1)<<16
    W2l = L(W2)
    all=W1h|W2l
    return all

def LFSRWithInitMode(u):#初始化模式
...
def S_(X):#s盒
    ...
    bit8[2]=(X>>8)&0xff
    bit8[3]=X&0xff
    #s盒由四个8输入8输出的的s盒组成，每个小s盒运行的规律为：输入一个8bit数据，前四位bit数据对应一个行值
    #后4位bit对应一个列值，由确定的行和列值得出一个新的8bit数据作为加密后的数据（在表中体现为十六进制）
    for i in range(4):
        row = bit8[i] >> 4
        nuw = bit8[i] & 0xf
        if i == 0 or i== 2:
...
def initialize(key,iv):#密钥装入和初始化
    global R1, R2, W,S
    for i in range(16):
        S[i] = (key[i] << 23) | (D[i] << 8) | iv[i]
    print('初始化之前线性移位寄存器基本状态为：')
    print_S()#打印线性移位寄存器的基本状态
    for i in range(32):#初始化阶段
        BitReconstruction()
        F(X[0], X[1], X[2])
...
def print_status():#打印xrws[15]的状态
    for i in range(4):
